    ...
    def _find(self, obj, insert_if_missing=False):
        ...
        while len(stack_obj) != 0:
            ...
            if obj not in self._child2parent:
                ...
                #print "objj {}".format(obj)
                #print "chidl {}".format(self._child2parent[obj])
                parent = self._child2parent[obj]
                if parent is not None:
                    stack_obj.append(parent)
                    stack_obj2.append(obj)
                else:
                    padre_todos = obj

        while len(stack_obj2) > 1:
            uno = stack_obj2.pop()
            if uno != padre_todos:
                self._join(uno,padre_todos)
        #print "exit"
        return padre_todos

    def _join(self, obj, parent=None):
        '''Join `obj` to its parent (or None if it's the root).'''
        if parent is not None and self._child2parent[parent] == obj:
            return
...
class BidirectionalEquivalence(Equivalence):
    ...
    def partition(self, obj):
        ...
        if obj in self._child2parent:
            stack_node = list()
            stack_node.append(self._find(obj))

            add = p.add
            get_children=self._parent2children.get
    ...
    def _join(self, obj, parent=None):
        ...
        if parent is not None and parent not in self._parent2children[obj] :
